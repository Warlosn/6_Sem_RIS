...
module.exports = (db) => {
    ...
    app.get('/listen', async function (req, res) {
        ...
        setInterval(() => {
            ...
            axios.get('http://192.168.43.99:8001/get')
                .then(res => {
        ...
        setInterval(() => {
            ...
            axios.get('http://192.168.43.99:8003/get')
                .then(res => {
    ...
    app.get('/set', async function (req, res) {
        axios.get('http://192.168.43.99:8001/update');
        axios.get('http://192.168.43.99:8003/update');
        res.json({});
    });
    ...
    app.post('/resource', async (req, res) => {
        ...
        if (item) {
            ...
            let response = await axios.get(`http://192.168.43.99:800${data.clNum === 1 ? 3 : 1}/get`);
    ...
    async function replicateDB(data)    //data - localhost:8003/get
    {
        for (let i = 0; i < data.length; i++) {
            ...
            if (item) {
                ...
                logger.debug('upd: ' + JSON.stringify({
                    'data': data[i].data,
                    'clNum': data[i].clNum,
                    'clNumId': data[i].id
                }));
                ...
                await db.model.create({'data': data[i].data, 'clNum': data[i].clNum, 'clNumId': data[i].id});
                logger.debug('add: ' + JSON.stringify({
                    'data': data[i].data,
                    'clNum': data[i].clNum,
                    'clNumId': data[i].id
                }));
